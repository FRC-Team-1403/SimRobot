...
public class Robot extends LoggedRobot {
  ...
  public void robotInit() {
    ...
    // DO NOT USE THIS CONFIG IN A REAL ROBOT!!!!!!!
    Logger.getInstance().recordMetadata("Sim", "Robot"); // Set a metadata value
    String logPath = LogFileUtil.findReplayLog(); // Pull the replay log from AdvantageScope (or prompt the user)
    Logger.getInstance().setReplaySource(new WPILOGReader(logPath)); // Read replay log
    Logger.getInstance().addDataReceiver(new WPILOGWriter(LogFileUtil.addPathSuffix(logPath, "_sim")));                                                                                         // a new log
